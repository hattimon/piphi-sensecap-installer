...
MESSAGES[pl,detecting_timezone]="Wykrywanie strefy czasowej z hosta..."
MESSAGES[pl,timezone_detected]="Wykryta strefa czasowa: %s"
...
MESSAGES[pl,gps_starting]="Uruchamianie gpsd w kontenerze..."
...
MESSAGES[en,detecting_timezone]="Detecting timezone from host..."
MESSAGES[en,timezone_detected]="Detected timezone: %s"
...
MESSAGES[en,gps_starting]="Starting gpsd in container..."
...
EOL
    ...
    # Run Ubuntu container with start-docker.sh as entrypoint (on host)
    msg "running_container"
    balena run -d --privileged -v /mnt/data/piphi-network:/piphi-network -p 31415:31415 -p 5432:5432 -p 3000:3000 --cpus="2.0" --memory="2g" --name ubuntu-piphi --restart unless-stopped ubuntu:20.04 /piphi-network/start-docker.sh || {
        msg "run_error"
    ...
    msg "gps_starting"
    exec_with_retry "ls /dev/ttyACM0 && gpsd /dev/ttyACM0" || {
        msg "gps_not_detected"
        balena logs ubuntu-piphi
        exit 1
    }
    ...
    # Install Docker without invoking systemd
    exec_with_retry "export DEBIAN_FRONTEND=noninteractive && apt-get install -y -o Dpkg::Options::=\"--force-confdef\" -o Dpkg::Options::=\"--force-confold\" docker-ce docker-ce-cli containerd.io docker-compose-plugin --no-install-recommends && systemctl disable docker.service docker.socket || true" || {
        msg "docker_error"
        balena logs ubuntu-piphi
        exit 1
    }
